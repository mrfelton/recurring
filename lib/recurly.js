...
function Recurring()
{
	...
	function handleRecurlyError(err, response, payload, validStatuses)
	{
		...
		if (idx === -1) {
			var msg = '';
			parser.parseXML(response.body, function(err, result) {
				if (err) {
					console.error('handleRecurlyError', 'xml parsing error ' + JSON.stringify(err));
				}
				msg = 'symbol: ' + result.symbol + ' description: ' + result.description;
			});
			return new Error('unexpected status: ' + response.statusCode + ' ' + msg);
		}
	}
